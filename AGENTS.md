...
This is a starter for AI Chessathon, a chess-engine competition. The deliverable is a zip
with `agent.py` at the archive root, exposing `get_move(fen, time_left_ms) -> str`. The
search lives in the `engine/` package. The zip is uploaded and the platform plays it
...
- `tools/` — pack, gauntlet, perft, smoke, and benches.
- `training/` — offline train and import scripts. Not part of the zip.
- `experiments/` — round-robin drivers, `RESULTS.md`, and small game logs.
- `docs/` — design notes. `archive/` — Stage 1 Python search.
- `local/engine_snapshots/` — historical engine trees. Gitignored.
- Pack with `python -m tools.pack` or `python -m harness.package --include engine` so `engine/`
  is in the zip. Root `*.py` other than `agent.py` should not be the engine modules.
...
- The environment is fixed. The platform preinstalls torch 2.13 (CPU), numpy 2.5, python-chess
  ...
  ignored, so an import outside that stack crashes on the platform even if it works locally.
...
- Do not use Stockfish, Lc0, Maia, or any existing engine inside the submission, including a
  ...
  inside the zip. Keep `tools/sf/` gitignored.
...
- Do not edit `harness/`. It mirrors the platform's protocol and clock. Changing it makes local
  results meaningless.
- Do not commit `tools/sf/`, `data/`, `local/`, `logs/`, or zip artefacts.
...
make zip       # build submission.zip with agent.py at the root and engine/
