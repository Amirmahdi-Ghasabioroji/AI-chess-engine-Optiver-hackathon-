"""Train v7.1.2 residual NNUE: target is Stockfish − classical on quiet positions.

    python -m training.train_v712_residual --from-scratch --device cuda
"""
...
V712 = ROOT / "local" / "engine_snapshots" / "v7.1.2"
if str(V712) not in sys.path:
    sys.path.insert(0, str(V712))
...
SCALE = 400.0
CLIP = 400.0
DEFAULT_OUT = V712 / "weights" / "nnue.npz"
DEFAULT_DATA = [
...
def rows_from_npz(paths: list[Path], max_resid: float) -> tuple[np.ndarray, np.ndarray]:
    seen: set[str] = set()
...
def main() -> int:
    ...
    parser.add_argument("--pretrained", type=Path, default=V712 / "weights" / "nnue.npz")
    ...
    features, labels = rows_from_npz(arguments.data, arguments.max_resid)
    n = features.shape[0]
    ...
    for extra in (V712 / "data" / "nnue.npz", V712 / "nnue.npz"):
