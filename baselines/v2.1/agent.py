...
import time. python-chess is used only at the edges: parsing the FEN we are sent
and proving that the move we send back is legal.
...
import bb_search
import book
from bbpos import from_board, move_to_uci
...
def _warmup() -> None:
    """Pay the JIT and table-build cost inside the 90s init budget.
    ...
    deadline = time.perf_counter() + 75.0
    bb_search.search(
        bbs,
        st,
        mb,
